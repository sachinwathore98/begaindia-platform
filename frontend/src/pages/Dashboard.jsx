...
export default function Dashboard() {
  ...
  // Dynamic States
  const [claimedPasses, setClaimedPasses] = useState(['EVT-101']);
  const [companyName, setCompanyName] = useState(user?.companyName || 'SW Digital Hub');
  const [category, setCategory] = useState('Digital & IT');
  const [mobile, setMobile] = useState(user?.mobile || '+91 8390876752');
  ...
  // Security & Real Brevo Email OTP State
  ...
  // Sync activeTab with URL path if direct sub-routes are accessed
  useEffect(() => {
    if (location.pathname.includes('/dashboard/profile')) setActiveTab('business');
    else if (location.pathname.includes('/dashboard/events')) setActiveTab('events');
    else if (location.pathname.includes('/dashboard/membership')) setActiveTab('membership');
    else if (location.pathname.includes('/dashboard/notifications')) setActiveTab('notifications');
  }, [location]);
  ...
  const handleSaveBusinessProfile = async (e) => {
    ...
    try {
      ...
      await axios.put(
        ...
        { companyName, category, mobile, city, description, logoUrl },
  ...
  // Real API Call to Brevo OTP Endpoint
  const handleSendOtp = async () => {
    try {
      setOtpLoading(true);
      setOtpMessage('');
      const res = await axios.post(`${API_URL}/api/auth/send-otp`, {
        email: user?.email || 'sachinwathore7698@gmail.com',
      });
      if (res.data.success) {
        setOtpSent(true);
        setOtpMessage('Verification OTP sent to your registered email!');
      }
    } catch (err) {
      setOtpMessage(err.response?.data?.message || 'Failed to dispatch email OTP. Using test mode.');
      setOtpSent(true);
    } finally {
      setOtpLoading(false);
    }
  ...
  // Check if we should render sub-route children via Outlet
  const isSubRoute = location.pathname !== '/dashboard' && location.pathname !== '/dashboard/';
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-64 bg-slate-900 text-slate-300 p-5 flex flex-col justify-between shrink-0 border-r border-slate-800">
        <div className="space-y-6">
          ...
          <nav className="space-y-1.5 pt-4">
            <button
              onClick={() => { setActiveTab('events'); navigate('/dashboard'); }}
              className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition ${
                activeTab === 'events' && !isSubRoute
            ...
            <button
              onClick={() => { setActiveTab('membership'); navigate('/dashboard'); }}
              className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition ${
                activeTab === 'membership' && !isSubRoute
                  ? 'bg-[#0A3D91] text-white shadow-sm font-bold'
                  : 'text-slate-400 hover:bg-slate-800 hover:text-white'
              }`}
            >
            ...
            <button
              onClick={() => { setActiveTab('business'); navigate('/dashboard'); }}
              className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition ${
                activeTab === 'business' && !isSubRoute
                  ? 'bg-[#0A3D91] text-white shadow-sm font-bold'
                  : 'text-slate-400 hover:bg-slate-800 hover:text-white'
              }`}
            >
              <Building2 className="w-4 h-4" /> Business Profile & Assets
            </button>

            <button
              onClick={() => { setActiveTab('notifications'); navigate('/dashboard'); }}
              className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition ${
                activeTab === 'notifications' && !isSubRoute
                  ? 'bg-[#0A3D91] text-white shadow-sm font-bold'
                  : 'text-slate-400 hover:bg-slate-800 hover:text-white'
              }`}
            >
              <Bell className="w-4 h-4 text-orange-400" /> Notifications & Broadcasts
            </button>

            <button
              onClick={() => { setActiveTab('overview'); navigate('/dashboard'); }}
              className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition ${
                activeTab === 'overview' && !isSubRoute
                  ? 'bg-[#0A3D91] text-white shadow-sm font-bold'
                  : 'text-slate-400 hover:bg-slate-800 hover:text-white'
              }`}
            >
      ...
      <main className="flex-1 p-6 md:p-8 space-y-6 max-w-6xl mx-auto w-full">
        ...
        {/* Render Outlet if navigating sub-pages, otherwise render inline tabs */}
        {isSubRoute ? (
          <Outlet />
        ) : (
          <>
              ...
              <div className="space-y-8">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Present these digital passes at entry verification desks for instant access.
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      Click <strong>"Get Pass"</strong> on any event to enrol and generate your entry pass.
                    </p>
                  </div>
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      Updates saved here will publish directly to the public BEGAINDIA Business Directory.
                    </p>
                  </div>
                  <Link
                    to="/directory"
            ...
            {/* TAB 5: SECURITY & REAL EMAIL OTP */}
            {activeTab === 'security' && (
              <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
                <div>
                  <h2 className="text-lg font-bold text-slate-900">Email OTP Verification & Security</h2>
                  <p className="text-xs text-slate-500 mt-1">
                    Request a real-time OTP sent to your registered email (<strong>{user?.email || 'sachinwathore7698@gmail.com'}</strong>) via Brevo API.
                  </p>
                </div>
                ...
                {otpMessage && (
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl text-xs text-[#0A3D91]">
                    {otpMessage}
                  </div>
                )}
                  ...
                  {!otpSent ? (
                    <div className="p-5 bg-slate-50 rounded-2xl border border-slate-200 space-y-3">
                      <p className="text-xs text-slate-700 font-medium">
                        Click below to dispatch an OTP code to your inbox.
                      </p>
                      <button
                        onClick={handleSendOtp}
                        disabled={otpLoading}
                        className="w-full py-2.5 bg-[#0A3D91] hover:bg-[#083278] text-white font-bold text-xs rounded-xl transition flex items-center justify-center gap-2 disabled:opacity-50"
                      >
                        <Mail className="w-4 h-4 text-[#F57C00]" />
                        {otpLoading ? 'Dispatching OTP...' : 'Send OTP to Email'}
                      </button>
                    </div>
                  ) : (
