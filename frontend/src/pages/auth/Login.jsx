...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Dispatch user details and JWT to Redux
      dispatch(setCredentials({ user: response.user, token: response.token }));
      
      // Redirect to Member Dashboard upon success
      navigate('/dashboard');
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-2xl border border-slate-200/80 shadow-sm p-8 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-500">Sign in to your BEGAINDIA Business Account</p>
        </div>
