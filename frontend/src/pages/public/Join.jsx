...
export default function Join() {
  ...
  const handleProceedPayment = async (e) => {
    ...
    try {
      ...
      // Step 2: Open Standard Razorpay Checkout
      const options = {
        ...
        handler: async function (response) {
          try {
            // Step 3: Verify Payment & Commit Account
      ...
      console.warn('Payment failed or test fallback activated', err);
      // Local fallback for offline/development test simulations
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-950 to-[#0A3D91]/20 py-12 px-4 sm:px-8 font-sans text-slate-100">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-3">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-500/10 border border-blue-400/20 text-[#F57C00] text-xs font-black uppercase tracking-widest">
            <Sparkles className="w-3.5 h-3.5" /> BEGA India Membership Onboarding
          </div>
        ...
        {!applicationCompleted && (
          <div className="flex items-center justify-center max-w-lg mx-auto py-2">
            <div className="flex items-center w-full justify-between relative">
              <div className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-slate-800 w-full -z-0"></div>
              <div
                className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-[#F57C00] transition-all duration-500 -z-0"
                style={{ width: `${((currentStep - 1) / 2) * 100}%` }}
              ></div>

              <div className={`relative z-10 flex flex-col items-center gap-1.5 ${currentStep >= 1 ? 'text-[#F57C00]' : 'text-slate-500'}`}>
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-black text-xs shadow-lg transition-all ${currentStep >= 1 ? 'bg-[#F57C00] text-slate-950 ring-4 ring-orange-500/20' : 'bg-slate-800 text-slate-400'}`}>
                  1
                </div>
                <span className="text-[10px] font-extrabold uppercase tracking-wider">Personal</span>
              </div>

              <div className={`relative z-10 flex flex-col items-center gap-1.5 ${currentStep >= 2 ? 'text-[#F57C00]' : 'text-slate-500'}`}>
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-black text-xs shadow-lg transition-all ${currentStep >= 2 ? 'bg-[#F57C00] text-slate-950 ring-4 ring-orange-500/20' : 'bg-slate-800 text-slate-400'}`}>
                  2
                </div>
                <span className="text-[10px] font-extrabold uppercase tracking-wider">Business</span>
              </div>

              <div className={`relative z-10 flex flex-col items-center gap-1.5 ${currentStep >= 3 ? 'text-[#F57C00]' : 'text-slate-500'}`}>
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-black text-xs shadow-lg transition-all ${currentStep >= 3 ? 'bg-[#F57C00] text-slate-950 ring-4 ring-orange-500/20' : 'bg-slate-800 text-slate-400'}`}>
                  3
                </div>
        ...
        {applicationCompleted && registeredData ? (
          <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 sm:p-12 shadow-2xl space-y-8 text-center animate-in zoom-in-95">
            ...
            <div className="space-y-2">
              <span className="px-3 py-1 bg-emerald-500/10 text-emerald-400 text-xs font-black rounded-full uppercase tracking-wider border border-emerald-500/20">
            ...
            {/* Render High-End Vector Digital ID Card */}
            <div className="max-w-md mx-auto bg-gradient-to-br from-slate-900 via-[#0A3D91] to-slate-950 border-2 border-amber-400/40 p-6 sm:p-8 rounded-3xl text-left shadow-2xl relative overflow-hidden space-y-6">
            ...
            <div className="flex flex-wrap justify-center gap-3 pt-4">
              <button
                ...
                className="px-6 py-3 bg-amber-400 hover:bg-amber-300 text-slate-950 font-black text-xs rounded-xl shadow-lg transition flex items-center gap-2"
              >
                <Printer className="w-4 h-4" /> Print / Save Badge PDF
              </button>
              <a
                href="/dashboard/membership"
                className="px-6 py-3 bg-[#0A3D91] hover:bg-[#083278] text-white font-extrabold text-xs rounded-xl shadow-lg transition flex items-center gap-2"
              >
          ...
          <div className="bg-slate-900/80 backdrop-blur-xl p-6 sm:p-10 rounded-3xl border border-slate-800 shadow-2xl">
            ...
            {currentStep === 1 && (
              <form onSubmit={handleNext} className="space-y-5 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1.5">Full Applicant Name *</label>
                    <input
                      type="text"
                      required
                      ...
                      className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white outline-none focus:border-[#F57C00] transition"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1.5">Mobile Contact *</label>
                    <input
                      type="tel"
                      required
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1.5">Login Password *</label>
                    <input
                      type="password"
                      required
                      minLength={6}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white outline-none focus:border-[#F57C00]"
                    >
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1.5">Taluka *</label>
                    <select
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1.5">Postal Address</label>
                  <input
                    type="text"
                ...
                <div className="flex justify-end pt-4">
                  <button
                    type="submit"
                    className="px-6 py-3 bg-[#0A3D91] hover:bg-[#083278] text-white text-xs font-extrabold rounded-xl shadow-lg transition flex items-center gap-2"
                  >
            ...
            {currentStep === 2 && (
              <form onSubmit={handleNext} className="space-y-5 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1.5">Business / Enterprise Name *</label>
                    <input
                      type="text"
                      required
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1.5">Legal Entity Structure</label>
                    <select
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1.5">GSTIN / Udyam Number (Optional)</label>
                    <input
                      type="text"
                      placeholder="27AAAAA0000A1Z5"
                      value={formData.gstNumber}
                      onChange={(e) => setFormData({ ...formData, gstNumber: e.target.value })}
                      className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white font-mono uppercase outline-none focus:border-[#F57C00]"
                    />
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1.5">Products / Services Overview</label>
                  <textarea
                    rows={3}
                ...
                <div className="flex justify-between pt-4">
                  <button
                    type="button"
                    onClick={handleBack}
                    className="px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold rounded-xl transition flex items-center gap-1.5"
                  >
            ...
            {currentStep === 3 && (
              <form onSubmit={handleProceedPayment} className="space-y-6 animate-in fade-in">
                <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                  <h3 className="text-base font-extrabold text-white flex items-center gap-2">
                    <CreditCard className="w-5 h-5 text-emerald-400" /> Step 3: Select Plan & Complete Payment
                  </h3>
                  <span className="text-xs text-slate-400 font-bold">Page 3 of 3</span>
                </div>
                    ...
                    return (
                      <div
                        ...
                        className={`p-5 rounded-2xl border-2 cursor-pointer transition flex flex-col justify-between gap-3 ${
                          isSelected
                            ? 'border-[#F57C00] bg-orange-500/10 shadow-lg shadow-orange-500/5'
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 pt-2 border-t border-slate-800">
                          ...
                          />
                          <span className="text-xs font-extrabold text-slate-300">Select Plan ({tier.fee})</span>
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className="p-4 bg-slate-950 border border-slate-800 rounded-2xl space-y-2">
                  <div className="flex items-start gap-2.5">
                    ...
                    />
                    <label htmlFor="declaration" className="text-xs text-slate-400 leading-relaxed">
                      I hereby apply for membership in <strong className="text-white">BEGA INDIA (व्यवसाय सक्षमीकरण व विकास संघटना)</strong>. I confirm all details are authentic and agree to adhere to the organization's Constitution and Code of Conduct.
                    </label>
                  </div>
                </div>
                ...
                <div className="flex justify-between pt-4">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="px-8 py-3.5 bg-gradient-to-r from-[#F57C00] to-amber-500 hover:from-[#e06f00] hover:to-amber-600 text-slate-950 font-black text-xs rounded-xl shadow-xl transition flex items-center gap-2"
                  >
