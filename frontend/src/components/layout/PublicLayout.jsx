...
export default function PublicLayout() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-800 font-sans">
      ...
      {/* Sticky Navigation Header with Expanded Height */}
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          
          {/* Larger Logo Section */}
          <Link to="/" className="flex items-center gap-3 group shrink-0">
            {/* Official BEGAINDIA PNG Logo with Larger Sizing (h-14 / 56px) */}
            <img
              src="/logo.png"
              alt="BEGAINDIA Logo"
              className="h-14 w-auto max-w-[200px] object-contain drop-shadow-xs transition-transform group-hover:scale-105"
              onError={(e) => {
                // If logo.png is missing or fails to load, gracefully show fallback badge
